...
一次完整使用通常是：打开 Dashboard 查看今日任务，完成 Agent 知识主题或记录 LeetCode 学习；提交知识检测后查看评分与反馈；在 Memory、Mastery 和 Review 页面确认薄弱点、掌握度与下一次复习安排。右下角 Web Floating Study Widget 会显示任务、进度、待检测项目和提醒。
...
没有配置 API Key 时，五个 Agent 都会使用确定性或模板 fallback，仍可演示计划、检测、评分、记忆和复盘流程。
...
| 变量 | 示例值 | 作用 |
| --- | --- | --- |
| `OPENAI_API_KEY` | `your_api_key` | OpenAI-compatible 服务的 API Key；可留空以使用 fallback |
...
API Key 只从环境变量读取，不写入数据库、Harness 或 Memory MCP。当前版本是单用户 MVP，使用 `user_id=default`；认证与租户隔离属于路线图。
...
### 五个 Agent 角色与 fallback

| Agent | 已实现职责 | 模型失败或不可用时的 fallback |
| --- | --- | --- |
| Goal Planner | 根据到期 Memory、掌握度和目标生成今日计划 | 确定性学习计划 |
| Quiz Agent | 根据知识主题和关键点生成检测问题 | 预设问题 |
| Evaluator Agent | 按 40/30/20/10 Rubric 评分并生成反馈 | Rule-based evaluator |
...
RAG 流程为：Markdown/Text → 标题解析 → 300～700 字符 Chunk（100 字符 overlap）→ Embedding → Chroma cosine 检索 → 来源约束回答。没有足够相似的 Chunk 时返回“知识库中没有足够依据”；模型调用失败时使用检索原文的引用式 fallback，不用外部知识补答。前端始终展示 sources 和 retrieved chunks。
...
3. **检索评估**：建立 retrieval hit rate、groundedness、引用准确率等可重复评估。
4. **桌面交付**：将 Web Floating Study Widget 交付为真正的桌面悬浮学习窗口。

## 贡献

欢迎围绕学习闭环、可审计性和安全边界提交 Issue 或 Pull Request。建议先说明问题场景和复现步骤，再提出与现有分层相匹配的改动；涉及 API、数据模型或 Agent 行为时，请同步补充测试和文档。提交前至少运行后端测试与前端生产构建：
